...
Q1BiYXNl = base64.b64decode('aHR0cDovL3Bhc3RlYmluLmNvbS9yYXcucGhwP2k9QWpVSG1DVzM=')

addon = xbmcaddon.Addon('plugin.video.3amigos')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.3amigosr3-%s]: %s" %(addon_version, string))
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(3 AMIGOS,New source added.,5000,"+icon+")")
        if not url is None:
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('3 AMIGOS','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('3 AMIGOS', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from 3 AMIGOS Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to 3 AMIGOS Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                ...
                contextMenu.append(('Add to 3 AMIGOS Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(3 AMIGOS,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(3 AMIGOS,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
