...
EXCLUDES     =  ['plugin.program.TotalRevolution','script.module.addon.common','addon.common.addon']
...
def Destroy_Path(path):
    dp.create("[COLOR=blue]T[/COLOR]otal[COLOR=dodgerblue]R[/COLOR]evolution","Wiping...",'', 'Please Wait')
...
def Kill_XBMC():
    dialog.ok('[COLOR=blue]T[/COLOR]otal[COLOR=dodgerblue]R[/COLOR]evolution','The system will now attempt to force close Kodi.','You may encounter a freeze, if that happens give it a minute','and if it doesn\'t close please restart your system.')
...
def Upload_Log(): 
    if ADDON.getSetting('email')=='':
        ...
        ADDON.openSettings()
    xbmc.executebuiltin('XBMC.RunScript(special://home/addons/plugin.program.TotalRevolution/uploadLog.py)')
#---------------------------------------------------------------------------------------------------
