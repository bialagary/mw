...
ADDON = xbmcaddon.Addon(id='script.tvguidetecbox')
...
class GuideTypes(object):
    ...
    filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvguidetecbox', 'guides.ini'))
    ...
    def __init__(self):
        try:
            ...
            xbmc.log('[script.tvguidetecbox] GuideTypes collected: %s' % str(self.guideTypes), xbmc.LOGDEBUG)
    ...
    def SectionMap(self, section):
        ...
        for option in options:
            try:
                ...
                if dict1[option] == -1:
                    xbmc.log('[script.tvguidetecbox] skip: %s' % option, xbmc.LOGDEBUG)
            except:
    ...
    def getGuideDataItem(self, id, item):
        ...
        try:
            ...
            xbmc.log('[script.tvguidetecbox] DataItem with index %s not found' % item, xbmc.LOGDEBUG)
        return value


    def getGuideById(self, id):
        xbmc.log('[script.tvguidetecbox] Finding Guide with ID: %s' % id, xbmc.LOGDEBUG)
        ret = []
        for guide in self.guideTypes:
            if guide[self.GUIDE_ID] == int(id):
                ret = guide
                xbmc.log('[script.tvguidetecbox] Found Guide with data: %s' % str(guide), xbmc.LOGDEBUG)
        return ret
...
if __name__ == '__main__':
    ...
    if ret >= 0:
        ...
        if xbmc.getCondVisibility('system.platform.android') and int(ver) < 15:
            ...
            xbmc.log('[script.tvguidetecbox] Running on ANDROID with Kodi v%s --> using workaround!' % str(ver), xbmc.LOGDEBUG)
            filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvguidetecbox', 'settings.xml'))
