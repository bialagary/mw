...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.tvguidetecbox] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.tvguidetecbox] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.tvguidetecbox] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.tvguidetecbox] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvguidetecbox'))
    KEY = 'xmltv'
    INI_TYPE_TEC = 0
    INI_TYPE_CUSTOM = 1
    INI_FILE = 'addons.ini'
    ...
    def __init__(self, addon):
        ...
        if self.logoSource == XMLTVSource.LOGO_SOURCE_TEC:
        ...
        if self.addonsType == XMLTVSource.INI_TYPE_TEC:
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            xbmc.log('[script.tvguidetecbox] Date to normalize: ' + dateString, xbmc.LOGDEBUG)
            try:
            ...
            t = t + td_local
            xbmc.log('[script.tvguidetecbox] Import Time adjusted from: ' + str(t_tmp) + ' to: ' + str(t), xbmc.LOGDEBUG)
            return t
