...
def doDownload(url, dest, title, referer, agent, cookie):
    ...
    if not resp:
        xbmcgui.Dialog().ok(title, dest, 'Preuzimanje neuspješno', 'Nema odgovora servera')
        return
    ...
    #print resp.headers
    if resumable:
        print "Preuzimanje se može nastaviti"

    if content < 1:
        xbmcgui.Dialog().ok(title, file, 'Nepoznata veličina', 'Nemoguće preuzeti')
        return
    ...
    if xbmcgui.Dialog().yesno(title + ' - Potvrdite preuzimanje', file, 'Cijela datoteka je %dMB' % mb, 'Nastavi s preuzimanjem?', 'Potvrdi',  'Prekini') == 1:
        return
    ...
    while True:
        ...
        if percent >= notify:
            xbmc.executebuiltin( "XBMC.Notification(%s,%s,%i)" % ( title + ' - Napredak preuzimanja - ' + str(percent)+'%', dest, 10000))
            ...
            if not chunk:
                if percent < 99:
                    ...
                    if not xbmc.Player().isPlaying():
                        xbmcgui.Dialog().ok(title, dest, '' , 'Preuzimanje završeno')
                    return
        except Exception, e:
            print str(e)
            error = True
        ...
        if (resumable and errors > 0) or errors >= 10:
            if (not resumable and resume >= 10) or resume >= 100:
                #Give up!
                print '%s preuzimanje prekinuto - previše pogreški prilikom preuzimanja' % (dest)
                xbmcgui.Dialog().ok(title, dest, '' , 'Preuzimanje neuspješno')
                return
