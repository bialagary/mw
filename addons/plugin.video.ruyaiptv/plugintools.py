...
def add_item( action="" , title="" , plot="" , url="" , thumbnail="" , fanart="" , show="" , episode="" , extra="", page="", info_labels = None, isPlayable = False , folder=True ):
    ...
    if url.startswith("plugin://"):
        ...
        try:
            ...
            qtitle = ""
        try:
            qurl = urllib.quote_plus(url)
        except:
            qurl = ""
        try:
            qthumbnail = urllib.quote_plus( thumbnail )
        except:
            qthumbnail = ""
        try:
            qplot = urllib.quote_plus( plot )
        except:
            qplot = ""
        try:
            qextra = urllib.quote_plus( extra )
        except:
            qextra = ""
        try:
            qpage = urllib.quote_plus( page )
        except:
            qpage = ""

        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' % ( sys.argv[ 0 ] , action , qtitle , qurl , qthumbnail , qplot , qextra , qpage)
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
    else:
