...
def ps(x):
	try:
		return {
			'__plugin__': 					"KissCartoon"
			,'__authors__': 				"[COLOR white]The[COLOR tan]Highway[/COLOR][/COLOR]"
			,'__credits__': 				""
			,'_addon_id': 					addon_id
			,'_plugin_id': 					addon_id
			,'_domain_url': 				"http://kisscartoon.me"
			,'_database_name': 			"kisscartoon"
			,'common_word': 				"Cartoon"
			,'common_word2': 				"Watch"
			,'_addon_path_art': 		"art"
			...
			,'img_kisslogo':				net.url_with_headers('http://kisscartoon.me/Content/images/logo.png')
			,'img_next':						net.url_with_headers('http://kisscartoon.me/Content/images/next.png')
			,'img_prev':						net.url_with_headers('http://kisscartoon.me/Content/images/previous.png')
			,'img_az':							net.url_with_headers('http://kisscartoon.me/Content/images/logo.png')
			,'img_search':					net.url_with_headers('http://kisscartoon.me/Content/images/read.png')
			,'img_hot':							net.url_with_headers('http://kisscartoon.me/Content/images/hot.png')
			,'img_updated':					net.url_with_headers('http://kisscartoon.me/Content/images/newupdate.png')
			,'GENRES_Notes': 				{'Dub':"English voices, no subtitle.",
															...
															'Sport':"Anime whose central theme revolves around sports, examples are tennis, boxing and basketball.",
			...
			,'changelog.url': 			'https://raw.github.com/HIGHWAY99/plugin.video.kisscartoon/master/changelog.txt'
			,'news.url': 						'https://raw.github.com/HIGHWAY99/plugin.video.kisscartoon/master/news.txt'
...
def psgn(x,t=".png"):
	...
	try:
		return {
			...
			,'crime': 					s+"wzuFudV"+t
			,'family': 					s+"Qmp6r09"+t
			,'a': 		s+"jK8WCQU"+t #OvFHLK2
			,'b': 		s+"S9IrzEd"+t #ezem9mn
			,'c': 		s+"EIezRxm"+t #707ILz1
			,'d': 		s+"nJaQuE3"+t #BUT7dUz
			,'e': 		s+"QJ5qMkU"+t #mzNtW2U
			,'f': 		s+"dU6Zma2"+t #11cykaC
			,'g': 		s+"nFc6QI6"+t #l0CvvHo
			,'h': 		s+"iDpK5PA"+t #VOupMGK
			,'i': 		s+"LJy6hSN"+t #ps3YPHq
			,'j': 		s+"W5glm55"+t #oNHwZWv
			,'k': 		s+"i2iOyeV"+t #TwHANG6
			,'l': 		s+"cVWbhdz"+t #xiuR2WX
			,'m': 		s+"1j9nVPQ"+t #GDEAPud
			,'n': 		s+"0bePtbD"+t #9FjSiMu
			,'o': 		s+"YuAMXpq"+t #TcR1pa0
			,'p': 		s+"6fbm9wg"+t #OGc4VBR
			,'q': 		s+"xc7rntM"+t #hL9tEkx
			,'r': 		s+"ALizHF4"+t #37NNHm8
			,'s': 		s+"rvvr24T"+t #mFQswUE
			,'t': 		s+"ZR193W3"+t #4DBQVrd
			,'u': 		s+"QJK67mj"+t #qpovLUW
			,'v': 		s+"kJSv1P1"+t #bnu5ByY
			,'w': 		s+"1GfqOin"+t #0IHoHV2
			,'x': 		s+"IO1I4Ul"+t #ic81iKY
			,'y': 		s+"mD28cOz"+t #46IlmRH
			,'z': 		s+"Auc71Eu"+t #PWUSCsE
			,'0': 		s+"nAUqPDG"+t #7an2n4W # 0RJOmkw
			,'all': 	s+"YIDeY9e"+t #hrWVT21
#			,'urlresolver settings': 			s+"PlROfSs"+t #PlROfSs #
			,'alphabetical': 							s+"QkD4eqE"+t #sddCXQo #
			,'latest update': 						s+"sQGighU"+t #dNCxQbg #
			,'most popular': 							s+"1xuoOzF"+t #T9LUsM2 #
			,'new '+ps('common_word').lower(): 								s+"rA6COOn"+t #BGZnMf5 #
			,'upcoming '+ps('common_word').lower(): 					s+"ZPkkfIz"+t #GXsQssP #4v2dThp
			,ps('common_word').lower()+' list': 							s+"JHWbp8G"+t #70X8AC3 #...
			,ps('common_word').lower()+' list all': 					s+"fIeFrLm"+t #5prowMg #t8b1hSX
			,ps('common_word').lower()+' list alphabet': 			s+"JIpBJSn"+t #IPg67Uz #R0w0BAM
			,ps('common_word').lower()+' list latest update': s+"DgfPUFC"+t #eJYEXPG #XG0LGQH
			,ps('common_word').lower()+' list newest': 				s+"O73jDtu"+t #jMlhsA0 #eWAeuLG
			,ps('common_word').lower()+' list popularity': 		s+"I3anmz7"+t #rLqjW8W #eTrguP1
			,'online bookmarks': 					s+"wRRe9Nf"+t #J4cq8kE #68ih1sx
			,'local change log': 					s+"uhuG2ga"+t #ZxF8Ots #f1nvgAM
			,'completed': 								s+"PDJjtxl"+t #Gfl5s2y #xcqaTKI
			,'favorites': 								s+"UbgTuuA"+t #MwBr7IK #lUAS5AU
			,'favorites 2': 							s+"qvFNOZu"+t #36zowiz #EA49Lt3
			,'favorites 3': 							s+"1E3WKZY"+t #jYkvtIE #lwJoUqT
			,'favorites 4': 							s+"SNNRYgs"+t #HFOzuDi #Wr7GPTf
			,'genre': 										s+"fPSni8Q"+t #xXWq96k #AmQHPvY
			,'genre select': 							s+"ROmqM07"+t #pyQ9PCj #MhNenb6
			,'menu history': 							s+"DRdRjjs"+t #NIVh5Ha #Kf32XmV
			,'history101': 								s+"eIU6H78"+t #Hpakbd1 #yQVdUcc
			,'history201': 								s+"q3UtDoK"+t #GV4WWB8 #fVuCkXg
			,'last': 											s+"qrNNDIj"+t #9GWLsvn #FelUdDz
			,'my list blank': 						s+"qDrw86U"+t #Qw2Ls2x #...
			,'my list add': 							s+"EU3oUgv"+t #IJXnNXp #oeMOOKx
			,'my list empty': 						s+"rveOQie"+t #z2Pv0jI #ZUQIDXk
			,'my list list': 							s+"9a2xAHS"+t #Vs91WzN #Eagu6rf
			,'my list': 									s+"AKq683y"+t #eTUfvMQ #pdMAaFb
			,'my list remove': 						s+"WVVr9M4"+t #2swQHRg #XBembzH
			,'my list user': 							s+"j5fgNPw"+t #ixz07hC #MDDPS0G
			,'ongoing': 									s+"Gw5EnNk"+t #eY0hS12 #EUak0Sg
			,'plugin settings': 					s+"bB7CYvW"+t #udcXDoJ #K4OuZcD
			,'search': 										s+"sFCPFJq"+t #JVRoCQ0 #mDSHRJX
			,'site shortcuts': 						s+"0ruzC2c"+t #aYSUxkN #w8aszrQ
			,'visited101': 								s+"lIiFg5W"+t #un1kLLM #dYOpFmD
			,'irc': 											s+"7YS28Z5"+".jpg"
			,'': 								s+""+t
#			,'': 								s+""+t
#			,'': 								s+""+t
...
### ############################################################################################################
### Other Settings ###
GENRES = ['Action','Adventure','Comedy','Crime','Drama','Family','Fantasy','Horror','Movie','Music','Musical','Mystery','Romance','Sci-Fi','Thriller']
