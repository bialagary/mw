...
WILDTVBase = 'aHR0cHM6Ly9naXRodWIuY29tL3RkYm56L1dpbGQtVFYvcmF3L21hc3Rlci9tZW51LW5ldw=='
...
addon = xbmcaddon.Addon('plugin.video.Wild TV')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.WILDTVr3-%s]: %s" %(string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(WILDTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(WILDTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def WILDTVIndex():
    addon_log("WILDTVIndex")
    addDir('[COLOR gold]Latest News[/COLOR]','Twitter',45,'http://www.stoneworksinternational.com/wp-content/uploads/2012/03/Latest-News.jpg' ,  FANART,'','','','')
    getData(base64.b64decode(WILDTVBase),'')

  
def twitter():
	text = 'Wild TV'
	twit = 'http://pastebin.com/raw.php?i=3f8QNUgM'
	...
	for status in match:
	    ...
	    text = status
	showText('[COLOR red][B]Latest News[/B][/COLOR]', text)
...
def WILDTVIndex():
    addon_log("WILDTVIndex")
    addDir('[COLOR red]Latest News[/COLOR]','facebook',45,'http://www.stoneworksinternational.com/wp-content/uploads/2012/03/Latest-News.jpg' ,  FANART,'','','','')
    getData(base64.b64decode(WILDTVBase),'')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(WILDTV,New source added.,5000,"+icon+")")
        addon.openSettings()
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('WILDTV Select A Source', List)
         if rNo>=0:
             rName=str(List[rNo])
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    if keyboard.isConfirmed():
       ...
       if len(Searchkey) == 0: 
          xbmcgui.Dialog().ok('WILDTV', 'Nothing Entered')
          return	   
    
    Searchkey = Searchkey.lower()
    List=[]
    List.append(WILDTVBase)
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
    progress = xbmcgui.DialogProgress()
    progress.create('WILDTV Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(WILDTV,Please enable Update Commonresolvers to Play in Settings. - ,10000)")
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('WILDTV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('WILDTV', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(WILDTV,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(WILDTV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
