...
ADDON = xbmcaddon.Addon(id='plugin.video.dextertv')
...
def addDir2(name,url,mode,iconimage):
        ...
        return ok
if active =="True":
 SOURCES = [{"url": "http://50.7.136.90/dev2/dex.php?username=%s&password=%s"%(usern,passw), "fanart": "http://dexteriptv.com/live/thumbs/fanart.jpg", "genre": "Tv Live", "date": "01.09.2015", "credits": "DexterTV", "thumbnail": "http://1.bp.blogspot.com/-USW8oQI2iPA/T-FssXUqJ-I/AAAAAAAAAIU/eqk5G_L5fRY/s1600/dexter.png"}]
else:
 SOURCES = [{"url": "http://50.7.136.90/dev2/index.html", "fanart": "http://dexteriptv.com/live/thumbs/fanart.jpg", "genre": "Tv Live", "date": "01.09.2015", "credits": "DexterTV", "thumbnail": "http://1.bp.blogspot.com/-USW8oQI2iPA/T-FssXUqJ-I/AAAAAAAAAIU/eqk5G_L5fRY/s1600/dexter.png"}]
 addDir('[COLOR oldlace][B]Click To Login[/COLOR][/B]','http://50.7.136.90:81/panel_api.php?username=%s&password=%s'%(usern,passw),28,'http://icons.iconarchive.com/icons/scoyo/badge/256/Login-icon.png')
 addDir('[COLOR blueviolet][B]Signup For A Free Account[/COLOR][/B]','http://50.7.136.90/dex_signup.php?username=%s&password=%s'%(usern,passw),29,'http://icons.iconarchive.com/icons/scoyo/badge/256/Signup-icon.png')
 addDir('[COLOR blueviolet][B]Recover Password[/COLOR][/B]','http://50.7.136.90/recover.php?username=%s'%(usern),30,'http://icons.iconarchive.com/icons/scoyo/badge/256/Signup-icon.png')
 addDir('[COLOR blueviolet][B]Forums:[/COLOR] [COLOR oldlace]http://dexteriptv.com/forums/index.php[/COLOR][/B]','',29,'https://www.seoclerk.com/pics/want29849-1I6ct01438641820.jpg')
...
def RECOVER():
    ...
    if dialog.yesno("Recover Password", 'You already need to be registered to use this','', "",'Cancel','Go'):
        email=Search('Email Address')
        recover=urllib2.urlopen('http://50.7.136.90/recover.php?username=%s'%(email)).read()
        if recover == 'nouser':
            dialog.ok("Error!", "No User Registered with that email!")
            return
	    if recover == 'novalid':
             dialog.ok("Error!", "Not a valid email")
             return
	    if recover == 'noemail':
             dialog.ok("Error!", "Email cant be empty")
             return
        else:
            dialog.ok("Success!", "We have sent your password to your email!")
...
def SIGNUP():
    dialog = xbmcgui.Dialog()
    if dialog.yesno("Free Signup", 'Do you Wish To Sign UP','', "",'Cancel','Sign UP'):
        email=Search('Valid Email(Will be checked)')
        addon.setSetting('usern',email)
        xbmc.executebuiltin('Container.Refresh')
        password=Search('Password')
        addon.setSetting('passw',password)
        xbmc.executebuiltin('Container.Refresh')
        signup=urllib2.urlopen('http://50.7.136.90/dex_signup.php?username=%s&password=%s'%(email,password)).read()
        if signup == 'OK':
            xbmc.executebuiltin('Container.Refresh')
            dialog.ok("Success!", "Your account has been created and a validation email has been sent to your email address")
            addon.setSetting('active','True')
            xbmc.executebuiltin('Container.Refresh')
            return
        elif signup =='duplicate':
            xbmc.executebuiltin('Container.Refresh')
            dialog.ok("Sorry!", "That email address is already in use.")
            addon.setSetting('active','False')
            xbmc.executebuiltin('Container.Refresh')
            return
        elif signup =='Nonpass':
            xbmc.executebuiltin('Container.Refresh')
            dialog.ok("Sorry!", "Password Can't Be Empty.")
            addon.setSetting('active','False')
            return
        elif signup =='Nonemail':
            xbmc.executebuiltin('Container.Refresh')
            dialog.ok("Sorry!", "Email Can't Be Empty.")
            addon.setSetting('active','False')
            return
        elif signup =='notemail':
            xbmc.executebuiltin('Container.Refresh')
            dialog.ok("Sorry!", "You need to use a valid email address.")
            addon.setSetting('active','False')
            return
        else:
            dialog.ok("Error!", " Signup Failed! Please try again")
            return
    else:
        return
def Search(name):
...
def SIGNIN():
    ...
    if dialog.yesno("Get Your Login Info Ready", 'Do you Wish To Sign In','', "",'Cancel','Sign In'):
        ...
        addon.setSetting('passw',password)
        xbmc.executebuiltin('Container.Refresh')
        login=urllib2.urlopen('http://50.7.136.90:81/panel_api.php?username=%s&password=%s'%(email,password)).read()
