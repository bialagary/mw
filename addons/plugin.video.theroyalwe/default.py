...
plugin.set_default_context_menu([
	...
	('Hide Watched', {'mode': 'toggle_hide_watched'}, True, validate_watched()==False),
	...
	*'''
	...
	class FourthPage(Window):
		def authorize(self):
			...
			response = TraktAPI().authorize(pin)
			if response:
...
def tv_genre():
	...
	if total_pages > 1: plugin.add_menu_item({'mode': 'tv_genre', "id": plugin.args['id'], "page": page+1}, {'title': '[COLOR blue]Next Page %s/%s >>[/COLOR]' % (page+1, total_pages)}, image=ARTWORK + "next_page.jpg")
	plugin.eod(VIEWS.LIST, 'tvshows')
plugin.register("tv_genre", tv_genre)
...
plugin.register(['tv_watchlist', 'tv_trending', 'tv_popular', 'tv_recommended', 'tv_custom_list', 'tv_search', 'tv_similar', 'tv_anticipated'], tv_list)
...
def season_list():
	...
	if imdb_id == 'None' or imdb_id is None:
		results = trakt.get_show_seasons(plugin.args['trakt_id'])
	else:
...
def episode_list():
	from dudehere.routines.trakt import TraktAPI
	trakt = TraktAPI()
	if plugin.args['imdb_id'] == 'None':
		show = trakt.get_show_info(plugin.args['trakt_id'])
		results = trakt.get_show_episodes(plugin.args['trakt_id'], plugin.args['season'])
	else:
	...
	for record in results:
		...
		if validate_transmogrifier():
			filename = "%s %sx%s" % (show['title'], record['season'], record['episode'])
		...
		if record['playcount'] == 0:
			menu.add('Mark Watched', {"mode": "set_watched", "media": "episode", "id": record['trakt_id'], "imdb_id": record['imdb_id']})
		else:
...
def calendar():
	...
	for record in results:
		...
		if validate_transmogrifier():
			filename = "%s %sx%s" % (record['showtitle'], record['season'], record['episode'])
			is_local = any(filename  in test for test in transmogrified)
		menu = ContextMenu()
		if record['playcount'] == 0:
			...
			menu.add('Mark Unwatched', {"mode": "set_unwatched", "media": "episode", "id": record['trakt_id']})
		menu.add('Find Similar Shows', {"mode": "tv_similar", "imdb_id": record['imdb_id']})
		menu.add('Go to Seasons', {"mode": "season_list", "imdb_id": record['imdb_id'], "trakt_id": record['trakt_id'], "fanart": record['backdrop_url']})
	...
	class Calendar(xbmcgui.WindowXMLDialog):
		...
		def show_context(self, controlID):
			...
			if action == 0:
				...
				query = {"mode": "season_list", "imdb_id": show['show']['ids']['imdb'], "fanart": show['show']['images']['fanart']['full']}
...
def movie_genre():
	...
	for result in results['results']:
		...
		if validate_transmogrifier():
			filename = "%s (%s)" % (movie_title, record['year'])
			is_local = any(filename  in test for test in transmogrified)
...
def movie_list():
	...
	if plugin.mode ==		'movie_watchlist':
	...
	ADDON.save_data('last_directory.qs', sys.argv[0]+sys.argv[2])
	plugin.eod(VIEWS.MOVIE_DEFAULT, 'movies')
plugin.register(['movie_watchlist', 'movie_trending', 'movie_popular', 'movie_recommended', 'movie_custom_list', 'movie_similar', 'movie_search'], movie_list)
...
def host_manager():
	...
	for host in hosts:
		...
		display = "[COLOR %s]%s | %s[/COLOR]" % (color, host['host'], host['weight'])
