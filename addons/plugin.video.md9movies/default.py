...
def CAT():
        ...
        #addDir('[B][COLOR white]TV Shows[/COLOR][/B]',baseurl+'/movie/filter/all/rating/all/all/all',2,icon,fanart,'')
...
def INDEX(url):
        ...
        all_videos = regex_get_all(link, '<a class="thumb"', '</h2>')
        ...
        for a in all_videos:
                ...
                year = regex_from_to(a, '<div class="year">', '<')
                if eps =='':
                        if metaset=='true':
                                addDir2('[B][COLOR white]%s (%s)[/COLOR][/B]' %(name,year),baseurl+url,3,icon,items)
                        else:
                                addDir('[B][COLOR white]%s (%s)(%s)[/COLOR][/B]' %(name,year,qual),baseurl+url,3,icon,fanart,'')
        try:
...
def EPIS(url):
        ...
        video_id = re.compile(r'updateMovieView\((.*?)\)').findall(link)[0]
        url =  'http://123movies.to/movie/loadepisoderss/'+str(video_id)+'/9'
        link = OPEN_URL(url)
        link = link.encode('ascii', 'ignore')
        all_videos = regex_get_all(link, '<item>', '</item>')
        for a in all_videos:
                name = regex_from_to(a, 'title>', '<').replace("&amp;","&").replace('&#39;',"'").replace('&quot;','"').replace('&#039;',"'")
                url = regex_from_to(a, 'file="', '"')
...
def SEARCH():
        ...
        if (keyb.isConfirmed()):
                ...
                for a in all_videos:
                        ...
                        year = regex_from_to(a, '<div class="year">', '<')
                        if eps == '':
                                addDir2('[B][COLOR white]%s[/COLOR][/B]' %name,baseurl+url,3,icon,items)
                        else:
                                addDir('[B][COLOR white]%s[/COLOR][/B]' %name,baseurl+url,6,icon,fanart,'')
