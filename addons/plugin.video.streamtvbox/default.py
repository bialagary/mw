...
STVBINI = os.path.join(ADDON.getAddonInfo('path'), 'resources', 'stvb.ini')
...
try:
   ...
   if not xbmcvfs.exists(file):
       stvb = xbmcaddon.Addon('plugin.video.streamtvbox')
...
if ADDON.getSetting('resetpass')=='true':
    dialog = xbmcgui.Dialog()
    if dialog.yesno(THESITE.upper(), "We Have Changed Our Site Payment System", "You Need To Reset Your [COLOR yellow]Password[/COLOR]",'Do You Want To do It Now ?',"No","Reset"):
        
        resetpass()

    else:
       if dialog.yesno(THESITE.upper(), "Have You Already Reset Your [COLOR yellow]Password[/COLOR] ?", "",'',"Yes","No"):

           dialog.ok(THESITE.upper(), "You Now Need To Reset", "Your [COLOR yellow]Password[/COLOR]")
           EXIT()
       else:
           ADDON.setSetting('resetpass','false')
...
cookie_jar = os.path.join(cookie_path, THESITE+"_LOGIN_AMEMBER_new_.lwp")
...
def downloadchannel():
    ...
    if sessionExpired():
        Login()
    net.set_cookies(cookie_jar)
    a=net.http_GET('http://'+THESITE+'/payments/MatrixUp/payments/api/matrix/channels',headers={'User-Agent' :UA}).content
    f = open(channeljs, mode='w')
    f.write(a)
    f.close()
...
def CATEGORIES():
    ...
    if os.path.exists(cookie_jar) == True:
        try:
            import random
            text=''
            twit = 'http://twitrss.me/twitter_user_to_rss/?user=@streamtvbox'
            link = OPEN_URL(twit)
        ...
        addDir('.[COLOR white]-------->>>>>>     [/COLOR][COLOR blue]My Subscription Status [/COLOR][COLOR white]     <<<<<<--------[/COLOR]','url',2006,'','','','')
        addDir('[COLOR gold].Mikey1234 Special Channels[/COLOR]','url',2003,image+'mikey.png','','','')
        addDir('[COLOR cyan].Upcoming Matches[/COLOR]','url',1999,image+'upcoming.png','','','')
        addDir('[COLOR plum].Premier League Pass[/COLOR]','http://streamtvbox.com/apisprotected/plp.php',2005,'','','','')
        addDir('[COLOR purple].American Sports Pass[/COLOR]','http://streamtvbox.com/apisprotected/usa.php',2005,'','','','') 
        addDir('[COLOR pink].Sports On Demand[/COLOR]','http://streamtvbox.com/apisprotected/channels1.php',2005,'','','','') 
        addDir('[COLOR green].On Demand[/COLOR]','https://raw.githubusercontent.com/Mercutio1/MercutioPlaylist/master/OSS%20Main',5,image+'ondemand.png','','','')
...
def PLAY_STREAM(name, url, iconimage, play, description):
    ...
    if sessionExpired():
        Login()
    net.set_cookies(cookie_jar)
    stream_url= net.http_GET('http://'+THESITE+'/payments/api/matrix/channel/%s'%url,headers={'User-Agent' :UA}).content
    if stream_url=='':
        return Show_Down()
    liz = xbmcgui.ListItem(name, iconImage='DefaultVideo.png', thumbnailImage=iconimage)
    liz.setInfo(type='Video', infoLabels={'Title':description})
